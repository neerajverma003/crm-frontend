...
const PreviewOfferLetter = ({ formatData, company, onClose }) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    pageStyle: `
      ...
      .offer-letter-section { margin: 0 !important; padding: 0 !important; box-shadow: none !important; width: 210mm !important; height: 297mm !important; min-height: unset !important; max-height: 297mm !important; overflow: hidden !important; display: flex !important; flex-direction: column !important; background-color: white !important; page-break-after: always !important; break-after: page !important; box-sizing: border-box !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      .offer-letter-section:last-child { page-break-after: auto !important; break-after: auto !important; }
      .section-header { flex-shrink: 0 !important; padding: 10mm 12mm 4mm 12mm !important; }
      .section-body { flex: 1 1 auto !important; overflow: hidden !important; padding: 0 12mm !important; }
      .section-footer { flex-shrink: 0 !important; padding: 4mm 12mm 8mm 12mm !important; }
      .rich-content p { font-size: 11px !important; line-height: 1.7 !important; color: #2d3748 !important; margin: 4px 0 !important; text-align: justify !important; }
      .rich-content h1, .rich-content h2, .rich-content h3, .rich-content h4 { font-weight: normal !important; color: #1a202c !important; margin-top: 8px !important; margin-bottom: 4px !important; }
      .rich-content ul, .rich-content ol { margin-left: 18px !important; margin-bottom: 6px !important; padding-left: 10px !important; }
      .rich-content li { margin-bottom: 3px !important; font-size: 11px !important; }
  ...
  /** One A4 page wrapper — screen: min-height card; print: exact 297mm page */
  const Section = ({ pageNumber, children }) => (
    <div
    ...
    >
      {/* Header */}
      <div className="section-header" style={{ flexShrink: 0, padding: "10mm 12mm 4mm 12mm" }}>
        <PageHeader />
      </div>

      {/* Body */}
      <div className="section-body" style={{ flex: 1, padding: "0 12mm", overflow: "visible" }}>
        {children}
      </div>

      {/* Footer */}
      <div className="section-footer" style={{ flexShrink: 0, padding: "4mm 12mm 8mm 12mm" }}>
  ...
  /** Numbered section heading — plain weight, no underline, matches PDF style */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      ...
      <style>{`
        ...
        @media print {
          ...
          /* Each section fills exactly one A4 page */
          ...
          .offer-letter-section:last-child {
        ...
        .rich-content li { font-size: 11px; line-height: 1.7; color: #2d3748; margin: 3px 0; padding-left: 6px; }
      ...
      <div className="relative w-full max-w-5xl max-h-screen bg-white rounded-lg shadow-2xl overflow-auto flex flex-col">
        ...
        {/* Modal Header */}
        <div className="no-print border-b border-slate-200 px-6 py-4 flex items-center justify-between bg-slate-50">
        ...
        {/* Action Buttons */}
        <div className="no-print border-b border-slate-200 px-6 py-3 bg-slate-50 flex gap-2">
          <button
            onClick={handlePrint}
        ...
        <div className="overflow-y-auto flex-1 p-6 bg-slate-100">
          <div ref={contentRef} style={{ backgroundColor: "#fff", fontFamily: "Arial, sans-serif", width: "100%", margin: "0 auto", padding: "0" }}>
            ...
            <Section pageNumber={1}>
              <div style={{ fontSize: "11px", lineHeight: "1.8", color: "#2d3748" }}>
                ...
                {/* Legal intro */}
                <p style={{ marginBottom: "15px", textAlign: "justify", fontSize: "11px", lineHeight: "1.7", pageBreakInside: "avoid" }}>
                  This Offer Letter is entered into between <strong>{company?.companyName || "the Company"}</strong> and{" "}
                  <strong>Mr. {formatData.candidateName || "[Name]"}</strong>,{" "}
                  {formatData.fatherName ? `S/o ${formatData.fatherName}, ` : ""}
                  residing at {formatData.candidateAddress || "[Address]"} (hereinafter referred to as the "Employee").
                </p>

                <p style={{ marginBottom: "12px", textAlign: "justify", fontSize: "11px", lineHeight: "1.7", pageBreakInside: "avoid" }}>
                ...
                <ol style={{ marginTop: "8px", marginBottom: "15px", paddingLeft: "20px", lineHeight: "1.7", fontSize: "11px", pageBreakInside: "avoid" }}>
                ...
                {formatData.jobResponsibilities && (
                  <div style={{ marginTop: "15px", pageBreakInside: "avoid" }}>
                    <h3 style={{ fontWeight: "normal", color: "#1a202c", fontSize: "11px", margin: "12px 0 8px 0" }}>Job Responsibilities:</h3>
            ...
            {/* ── PAGE 2: All Terms & Conditions (4-13) + Signature ── */}
            <Section pageNumber={2}>
              <div style={{ fontSize: "11px", lineHeight: "1.8", color: "#2d3748" }}>
            ...
            {formatData.nonDisclosureAgreement && (
              <>
                {/* NDA PAGE 1: Title + intro + sections 1–5 */}
                <Section pageNumber={ndaStartPage}>
                  <div style={{ fontSize: "11px", lineHeight: "1.8", color: "#2d3748" }}>
                    ...
                    {formatData.ndaWhereas && (
                      <div style={{ marginBottom: "12px" }}>
                        <p style={{ fontWeight: "normal", marginBottom: "6px" }}>WHEREAS</p>
                    ...
                    {[
                      ...
                      formatData[key] ? (
                        <div key={key} style={{ marginBottom: "10px" }}>
                          <p style={{ fontWeight: "normal", marginBottom: "4px" }}>{label}</p>
                ...
                {/* NDA PAGE 2: Sections 6–8 + signatures */}
                <Section pageNumber={ndaStartPage + 1}>
                  <div style={{ fontSize: "11px", lineHeight: "1.8", color: "#2d3748" }}>
                    {[
                      ...
                      formatData[key] ? (
                        <div key={key} style={{ marginBottom: "12px" }}>
                          <p style={{ fontWeight: "normal", marginBottom: "6px" }}>{label}</p>
                          <RichContent html={formatData[key]} />
                        </div>
                      ) : null
                    )}
