    ...
    <div className="max-w-6xl mx-auto my-6 sm:my-10 px-4 sm:px-6 lg:px-8">
      <div className="bg-white shadow-sm border border-slate-200 rounded-3xl p-6 sm:p-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 mb-8">
          <div>
            <h2 className="text-2xl sm:text-3xl font-extrabold text-blue-900 tracking-tight">Create State</h2>
            <p className="text-slate-500 font-medium mt-1">Manage domestic and international states for operations</p>
          </div>
        </div>
          ...
          <div>
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                ...
                {/* STATE */}
                <div className="w-full">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">State Name</label>
                  <input
                    type="text"
                    name="stateName"
          ...
          <div>
            ...
            {filteredData.length === 0 ? (
              <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center">
                ...
                <p className="text-slate-500 font-bold text-lg">No states found</p>
                <p className="text-slate-400 text-sm mt-1">Add a new state using the form above.</p>
              </div>
            ) : (
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filteredData.map((d, index) => (
                        <tr key={d._id || index} className="hover:bg-blue-50/30 transition-colors">
                          ...
                          <td className="px-6 py-4 font-bold text-blue-900 text-base">{d.state}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  {filteredData.map((d, index) => (
                    <div key={d._id || index} className="p-4 flex flex-col gap-2 hover:bg-slate-50 transition-colors">
                      ...
                      <div className="mt-1">
                        ...
                        <div className="text-lg font-black text-slate-800">{d.state}</div>
                      </div>
                    </div>
                  ))}
                </div>
