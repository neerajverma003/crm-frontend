...
const SuperAdminAttendance = () => {
    ...
    const handleSaveDateAttendance = async (updatedData) => {
        ...
        try {
            if (selectedDateForEdit.records && selectedDateForEdit.records.length > 0) {
                ...
                // Create new record (only for employees, not admins)
                if (selectedUser?.userType === "Admin") {
                    alert("Cannot create new attendance record for admin. Please ensure attendance exists first.");
                    return;
                }
    ...
    const calculateAttendanceStats = (attendanceData, month) => {
        ...
        stats.total = stats.present + stats.gracePresent + stats.late;
