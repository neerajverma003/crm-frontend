...
const SuperAdminAttendance = () => {
    ...
    const handleSaveDateAttendance = async (updatedData) => {
        ...
        try {
            if (selectedDateForEdit.records && selectedDateForEdit.records.length > 0) {
                ...
                // if admin then hit the admin attendance api 
                if (selectedUser?.userType === "Admin") {
                    url = `${import.meta.env.VITE_API_URL}/adminAttendance/create`;
                    bodydata = {
                        admin: selectedUser._id,
                        date: new Date(selectedDateForEdit.date),
                        ...updatedData
                    };
                }
    ...
    const DateAttendanceEditModal = ({ date, dayRecords, onClose, onSave }) => {
        ...
        const [clockOut, setClockOut] = useState(() => {
            if (dayRecords && dayRecords.length > 0) {
                return formatLocalDateTime(dayRecords[0]?.clockOut);
            }
